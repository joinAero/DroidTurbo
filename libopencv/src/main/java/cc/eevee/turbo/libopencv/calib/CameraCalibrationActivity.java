...
package cc.eevee.turbo.libopencv.calib;
...
public class CameraCalibrationActivity extends AppCompatActivity implements CvCameraViewListener2, OnTouchListener {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (i == R.id.calibration) {
            mOnCameraFrameRender =
                    new OnCameraFrameRender(new CalibrationFrameRender(mCalibrator));
            item.setChecked(true);
            return true;
        } else if (i == R.id.undistortion) {
            mOnCameraFrameRender =
                    new OnCameraFrameRender(new UndistortionFrameRender(mCalibrator));
            item.setChecked(true);
            return true;
        } else if (i == R.id.comparison) {
            mOnCameraFrameRender =
                    new OnCameraFrameRender(new ComparisonFrameRender(mCalibrator, mWidth, mHeight, getResources()));
            item.setChecked(true);
            return true;
        } else if (i == R.id.calibrate) {
            final Resources res = getResources();
            if (mCalibrator.getCornersBufferSize() < 2) {
                (Toast.makeText(this, res.getString(R.string.ocv_more_samples), Toast.LENGTH_SHORT)).show();
                return true;
            }
            ...
            new AsyncTask<Void, Void, Void>() {
                ...
                protected void onPreExecute() {
                    ...
                    calibrationProgress.setTitle(res.getString(R.string.ocv_calibrating));
                    calibrationProgress.setMessage(res.getString(R.string.ocv_please_wait));
                ...
                protected void onPostExecute(Void result) {
                    ...
                    String resultMessage = (mCalibrator.isCalibrated()) ?
                            res.getString(R.string.ocv_calibration_successful) + " " + mCalibrator.getAvgReprojectionError() :
                            res.getString(R.string.ocv_calibration_unsuccessful);
